#' Get Gene Overlaps with CNVkit segments
#'
#' @param segs.df data.frame of the CNVkit segment file (.cns).
#' @param gene.annot.df data.frame of the gene annotations.
#' @param overlap.prop Proportion that the segment must overlap with
...
get_gene_titan_seg_overlap <- function(segs.df, gene.annot.df, overlap.prop = 0.5) {
  ...
  overlap.res <- GenomicRanges::findOverlaps(gene.annot.gr, seg.gr)
  gene.annot.gr.hits <- S4Vectors::queryHits(overlap.res)
  seg.gr.hits <- S4Vectors::subjectHits(overlap.res)

  gene.annot.gr.overlap <- gene.annot.gr[gene.annot.gr.hits, ]
  seg.gr.overlap.mcols <- 
    S4Vectors::mcols(seg.gr[seg.gr.hits, ])
